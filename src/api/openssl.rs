...
impl CallBuilder {
    ...
    pub fn call(&mut self, httpc: &mut Httpc, poll: &Poll) -> ::Result<Call> {
        ...
        httpc.call::<tls_api::openssl::TlsConnector>(cb, poll)
    }
    pub fn websocket(&mut self, httpc: &mut Httpc, poll: &Poll) -> ::Result<::WebSocket> {
        let mut cb = self.cb.take().unwrap();
        cb.websocket();
        let cid = httpc.call::<tls_api::openssl::TlsConnector>(cb, poll)?;
...
impl Httpc {
    ...
    pub fn event(&mut self, ev: &Event) -> Option<CallRef> {
        self.h.event::<tls_api::openssl::TlsConnector>(ev)
    }
    pub fn call_send(&mut self, poll: &Poll, id: &mut Call, buf: Option<&[u8]>) -> ::SendState {
        self.h
            .call_send::<tls_api::openssl::TlsConnector>(poll, id, buf)
    }
    pub fn call_recv(
        &mut self,
        poll: &Poll,
        ...
        self.h
            .call_recv::<tls_api::openssl::TlsConnector>(poll, id, buf)
    }
}
